...
def main() -> None:
    ...
    answer_json = source / "blacklabel_answers.json"
    if answer_json.is_file():
        entries.append(
            {
                "source": str(answer_json),
                "object": f"{TEXTBOOK_ID}/blacklabel_answers.json",
                "bytes": answer_json.stat().st_size,
                "sha256": sha256(answer_json),
            }
        )
    ...
    print(f"준비 완료: {len(images)}개 문제" + (" + 빠른정답 JSON 1개" if answer_json.is_file() else ""))
